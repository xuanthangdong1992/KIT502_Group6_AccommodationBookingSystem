...
if ($_POST['action']=="change_info") {
    ...
    $query = "UPDATE `account` SET `first_name`='$firstName',`last_name`='$lastName',`email`='$email',`phone_number`='$phoneNumber',`postal_address`='$postalAddress', `abn_number`='$abnNumber' WHERE `user_id`='$user_id'";
    if ($conn->query($query)) {
        echo 'success';
    } else {
        echo 'fail';
